...
usage() {
    ...
    echo "projid,<PROJID>"
    echo "email,<email for recipient>"
...
if [ -f $delinfo ]; then
    ...
    if [ -z "$email" ]; then
	echo "> Error: no recipient email found in $delinfo.. this file needs a row with 'email,<email adress>'"
	exit_abnormal
    fi 
    if [ -z $pid ]; then
...
# Check if no customer specified- then mv CC emails (above) as TO email
if [ -z "$email" ]; then
...
# If sc-cite-seq-10x : attach websummaries
att=""
if [[ "$pipe" == "sc-cite-seq-10x" ]]; then
    # attach web summaries (.tar.gz)
    echo "> sc-cite-seq-10x delivery: tar zip web-summaries for attaching to mail.."
...
mailcmd="echo '' | mutt -s 'CTG $pipe delivery of $pid' $email -i /srv/data/$customer/$newmail -e 'unmy_hdr from; my_hdr From: ${ctgName} <${ctgmail}>' -e 'set content_type=text/html' ${cc} $att"
...
# Copy in feature ref table
# Generate html table from metadata/feature.ref.csv
htmltab=$(bash /projects/fs1/shared/ctg-pipelines/ctg-sc-cite-seq-10x/v2.0/bin/csv2html.sh $data/metadata/feature.ref.csv | tr -d '\n\r')
sed "s|xxfeaturerefxx|$htmltab|g" ctg-delivery-mail_sc-cite-seq-10x.csv > tmp.txt; mv tmp.txt $newmail
